...
WRAPPER void CTimeCycle__Update(void) { EAXJMP(0x4CEA40); }
...
byte &clockHour = *(byte*)0xA10B6B;
byte &clockMinute = *(byte*)0xA10B92;
byte &clockSecond = *(byte*)0xA10A3C;
short &oldWeather = *(short*)0xA10AAA;
short &newWeather = *(short*)0xA10A2E;
float &weatherInterp = *(float*)0x9787D8;

// car tweak
static float fresnelTable[24][NUMWEATHER], currentFresnel;
static float specPowerTable[24][NUMWEATHER], currentSpecPower;
static RwRGBAReal diffuseTable[24][NUMWEATHER], currentDiffuse;
static RwRGBAReal specularTable[24][NUMWEATHER], currentSpecular;

// rim tweak
static RwRGBAReal rampStartTable[24][NUMWEATHER], currentRampStart;
static RwRGBAReal rampEndTable[24][NUMWEATHER], currentRampEnd;
static float offsetTable[24][NUMWEATHER], currentOffset;
static float scaleTable[24][NUMWEATHER], currentScale;
static float scalingTable[24][NUMWEATHER], currentScaling;
...
{
	float f;
	float (*t)[24][NUMWEATHER] = (float (*)[24][NUMWEATHER])table;
	sscanf(s, "%f", &f);
	(*t)[line][field]  = f;
}
...
{
	int r, g, b, a;
	RwRGBAReal (*t)[24][NUMWEATHER] = (RwRGBAReal (*)[24][NUMWEATHER])table;
...
readDatFiles(void)
{
	...
	// make it update
	MemoryVP::InjectHook(0x4A45F5, updateTweakValues);
}
